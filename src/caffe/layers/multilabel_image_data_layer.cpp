...
void MultilabelImageDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  string root_folder = this->layer_param_.multilabel_image_data_param().root_folder();
	const int num_of_label = this->layer_param_.multilabel_image_data_param().num_of_label();
  ...
  vector<int> top_shape = this->data_transformer_->InferBlobShape(cv_img);
	//std::cout << top_shape[0] << ' ' << top_shape[1] << ' ' << top_shape[2] << ' ' << top_shape[3] << std::endl;
  ...
  vector<int> label_shape;
	...
	label_shape.push_back(num_of_label + 1);
...
void MultilabelImageDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  string root_folder = multilabel_image_data_param.root_folder();
	const int num_of_label = multilabel_image_data_param.num_of_label();
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    prefetch_label[item_id * (num_of_label + 1) + num_of_label] = lines_[lines_id_].second.size();
		for (size_t lable_id = 0; lable_id < lines_[lines_id_].second.size(); lable_id++){
			prefetch_label[item_id * (num_of_label + 1) + lable_id] = lines_[lines_id_].second[lable_id];
		}
    // go to the next iter
    lines_id_++;
