  ...
  */
  ...
  for (int frame_id = 0; frame_id < frame_files.size(); ++frame_id) {
    ...
    cv::resize(cv_img_orig, cv_img, cv::Size(image_size, image_size));
		...
		for (int crop_id = 0; crop_id < num_of_crop; crop_id++) {
			height_offset = caffe_rng_rand() % (image_size - image_crop_size + 1);
			width_offset = caffe_rng_rand() % (image_size - image_crop_size + 1);
			...
			for (int i_channel = 0; i_channel < n_channel; ++i_channel) {
				for (int height = 0; height < image_crop_size; ++height) {
					for (int width = 0; width < image_crop_size; ++width) {
						int insertion_index = crop_id * n_image_crop_pixel + (i_channel * image_crop_size + height) * image_crop_size + width;
    ...
    //if ((frame_id + 1) % batch_size == 0 || (frame_id + 1) == frame_files.size()) {
      ...
      for (int i_layer = 0; i_layer < n_layer; ++i_layer) {
        ...
        const int n_example = data_blob->num();
				const int n_channel = data_blob->channels();
				//fwrite(data_blob_ptr, sizeof(float), n_example * n_channel, output_files[i_layer]);
				
				float* tmp_data = new float[n_channel];
				//cout << "n_example: " << n_example << ", n_channel: " << n_channel << endl;
				for(int j=0; j < n_channel; j++) tmp_data[j] = 0;
				for(int i=0; i < n_example; i++){ 
					for (int j=0; j < n_channel; j++) {
						tmp_data[j] += data_blob_ptr[i * n_channel + j];
					}
				}
				for(int j=0; j < n_channel; j++) tmp_data[j] /= n_example;
				
				fwrite(tmp_data, sizeof(float), n_channel, output_files[i_layer]);
				
			}

      LOG(INFO) << frame_id + 1 << " (+" << batch_frame_names.size()
